import { ProjectScope } from "../../features/workspace/project_scope";
/**
...
export class IdbAdapter {
  ...
  public static async put<T extends BaseEntity>(storeName: string, entity: T): Promise<T> {
    ...
    entity = ProjectScope.prepare(storeName, entity, scope);
    const db = await this.getDb();
    if (ProjectScope.enabled && scope !== ProjectScope.id) throw new Error("Le chantier actif a changé.");
    return new Promise((resolve, reject) => {
      ...
      let failure = `Écriture annulée dans ${storeName}`;
      const previous = store.get(record.id);
      previous.onsuccess = () => {
        const existing = previous.result;
        if (ProjectScope.enabled && existing &&
            ((!ProjectScope.visible(storeName, existing, scope)) ||
             (storeName === 'projects' && ['CLOTURE','ANNULE'].includes(existing.status)))) {
          failure = 'Enregistrement inaccessible ou chantier clôturé.';
          transaction.abort(); return;
        }
        store.put(record);
      };
      transaction.oncomplete = () => resolve(record);
      transaction.onabort = () => reject(new DatabaseException(failure));
    });
  }
  ...
  public static async getById<T extends BaseEntity>(storeName: string, id: string): Promise<T | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (!item || item.deletedAt || !ProjectScope.visible(storeName,item)) {
          resolve(null);
        } else {
          resolve(item);
        }
   ...
   */
  public static async getAll<T extends BaseEntity>(storeName: string, includeDeleted = false): Promise<T[]> {
    const db = await this.getDb();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction([storeName], "readonly");
      const store = transaction.objectStore(storeName);
      ...
      request.onsuccess = () => {
        const items = (request.result as T[]).filter((item) => (includeDeleted || !item.deletedAt) && ProjectScope.visible(storeName,item));
        resolve(items);
      };
  ...
  public static async hardDelete(storeName: string, id: string): Promise<boolean> {
    if(ProjectScope.enabled && ProjectScope.scopedStores.has(storeName)) {
      const item=await this.getById(storeName,id);
      if(!item) throw new Error("Opération inaccessible dans ce chantier.");
      ProjectScope.prepare(storeName,item);
    }
    const db = await this.getDb();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction([storeName], "readwrite");
      ...
      transaction.onabort = () => reject(new DatabaseException(`Suppression annulée dans ${storeName}`));
      request.onerror = () => reject(new DatabaseException(`Erreur suppression dans ${storeName}: ${request.error?.message}`));
    });
  }

  /** Administrative enumeration for project selection and complete backup. */
  public static async getAllUnscoped<T extends BaseEntity>(storeName:string):Promise<T[]> {
    const db=await this.getDb();
    return new Promise((resolve,reject)=>{const r=db.transaction(storeName,'readonly').objectStore(storeName).getAll();r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);});
  }
