...
import { AppPermission as P } from "../core/permissions/permissions";
import { ROUTE_PERMISSIONS } from "../core/permissions/route_permissions";
...
export const AppRouter: React.FC = () => {
  ...
  const { currentUser, isAuthenticated, isLoading, isFirstLoginModalRequired, hasPermission } = useAuth();
  ...
  const renderRouteContent = () => {
    if (isLoading) return <p>Chargement de la session…</p>;
    if (!isAuthenticated) return <AuthPortalScreen onLoginSuccess={handleLoginSuccess} />;
    if (isFirstLoginModalRequired) return <p>Veuillez terminer votre première connexion.</p>;
    const required = ROUTE_PERMISSIONS[currentRoute];
    if (required && !required.every(hasPermission)) {
      return <AppEmptyState title="Accès refusé" description="Votre rôle ne permet pas de consulter ce module." />;
    }
