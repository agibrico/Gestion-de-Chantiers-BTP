...
export class FinanceRepositoryImpl {
  ...
  public static async createExpense(
    ...
    await this.init();
    const now = new Date().toISOString();
    if (!Number.isSafeInteger(data.amountFCFA) || data.amountFCFA <= 0) throw new Error("Le montant FCFA doit être un entier positif.");
    if (!data.projectId?.trim() || !data.title?.trim()) throw new Error("Chantier et libellé obligatoires.");
  ...
  public static async getAllCashTransactions(): Promise<CashTransactionEntity[]> {
    return []; // No fabricated cash transactions; cash ledger remains to implement.
  }
}
