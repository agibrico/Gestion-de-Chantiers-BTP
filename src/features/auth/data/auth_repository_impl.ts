...
export class AuthRepositoryImpl implements IAuthRepository {
  ...
  public async initializeSeedData(): Promise<void> {
    if (!this.seedPromise) {
      this.seedPromise = this.seedData().finally(() => { this.seedPromise = null; });
    }
  ...
  public async login(phone: string, password: string): Promise<UserEntity> {
    ...
    const valid = user.passwordHash?.startsWith("pbkdf2$")
      ? await verifyPassword(password, user.passwordHash)
      : user.passwordHash === password; // Migration on successful legacy login only.
    if (!valid) {
  ...
  public async completeFirstLogin(
    ...
    if (!newPassword || newPassword.trim().length < 8) {
      throw new AuthException("Le nouveau mot de passe doit comporter au moins 8 caractères.");
    }
    ...
    const updatedUser: UserEntity = {
      ...
      identityDocument: identityDoc?.documentNumber?.trim() ? {
        ...identityDoc,
        verified: false,
  ...
  public async createEmployee(dto: CreateEmployeeDTO): Promise<UserEntity> {
    ...
    const newEmployee: UserEntity = {
      ...
      passwordHash: await hashPassword("1234"), // Mot de passe par défaut
