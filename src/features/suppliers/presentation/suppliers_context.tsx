...
export const SuppliersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const markAsDelivered = async (
    ...
    try {
      await repository.markAsDelivered(id, blNumber, date, deliverySignature);
      toast.success(
        "Livraison réceptionnée & signée",
        `Rapprochement BL ${blNumber} validé avec émargement manuscrit.`
      );
      await loadData();
    } catch (e) {
