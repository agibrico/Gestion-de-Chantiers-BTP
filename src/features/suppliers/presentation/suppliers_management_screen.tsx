...
import { DeliverySignatureModal } from "./delivery_signature_modal";
import { ViewSignedBlModal } from "./view_signed_bl_modal";
...
const SuppliersManagementContent: React.FC = () => {
  ...
  const [orderForSignature, setOrderForSignature] = useState<PurchaseOrderEntity | null>(null);
  const [orderToViewBl, setOrderToViewBl] = useState<PurchaseOrderEntity | null>(null);
  ...
  const handleDeliver = (ord: PurchaseOrderEntity) => {
    setOrderForSignature(ord);
  };
  ...
  return (
    <div className="space-y-6">
                      ...
                      <tr key={ord.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="p-3.5">
                          {ord.deliveryNoteNumber ? (
                            <button
                              onClick={() => setOrderToViewBl(ord)}
                              className="font-mono text-[11px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 px-2 py-0.5 rounded border border-emerald-300 dark:border-emerald-800 cursor-pointer flex items-center gap-1"
                              title="Voir le Bon de Livraison et l'émargement manuscrit"
                            >
                              <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                              {ord.deliveryNoteNumber}
                              <span className="text-[9px] bg-emerald-200 dark:bg-emerald-900 px-1 rounded">Signé</span>
                            </button>
                          ) : (
                        ...
                        <td className="p-3.5 text-right space-x-1">
                          ...
                          {(ord.status === "VALIDE_DIRECTION" || ord.status === "COMMANDE_ENVOYEE") && (
                            <button
                              onClick={() => handleDeliver(ord)}
                              className="px-2.5 py-1 text-[10px] font-bold bg-orange-600 hover:bg-orange-700 text-white rounded cursor-pointer shadow-xs inline-flex items-center gap-1"
                              title="Réceptionner livraison & signer tactilement le BL"
                            >
                              <span>✍️</span> Signer BL
                            </button>
                          )}
                          {ord.status === "LIVRE_CONFORME" && (
                            <button
                              onClick={() => setOrderToViewBl(ord)}
                              className="px-2 py-1 text-[10px] font-bold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded cursor-pointer"
                              title="Consulter le BL signé"
                            >
                              Voir BL
                            </button>
                          )}
      ...
      {/* Delivery Signature Touchscreen Modal */}
      <DeliverySignatureModal
        isOpen={!!orderForSignature}
        onClose={() => setOrderForSignature(null)}
        order={orderForSignature}
        onConfirmDelivery={async (orderId, blNumber, deliveryDate, sigData, driverName) => {
          await markAsDelivered(orderId, blNumber, deliveryDate, {
            deliverySignatureDataUrl: sigData.signatureDataUrl,
            deliverySignatoryName: sigData.signatoryName,
            deliverySignatoryRole: sigData.signatoryRole,
            deliverySignedAt: sigData.timestamp,
            driverSignatoryName: driverName,
          });
        }}
      />

      {/* View Signed BL Receipt Modal */}
      <ViewSignedBlModal
        isOpen={!!orderToViewBl}
        onClose={() => setOrderToViewBl(null)}
        order={orderToViewBl}
      />
    </div>
  );
