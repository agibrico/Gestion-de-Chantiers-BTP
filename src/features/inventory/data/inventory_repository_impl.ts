...
export class InventoryRepositoryImpl implements InventoryRepository {
  ...
  public async seedInitialInventoryIfEmpty(): Promise<void> {
    return; // User-created data only; existing records preserved.
    try {
  ...
  public async recordMovement(
    ...
    movementData = ProjectScope.prepare("stock_movements",movementData);
    const scope=ProjectScope.id;
    const isAdjustment = movementData.movementType === "AJUSTEMENT_INVENTAIRE";
    if (!Number.isFinite(movementData.quantity) || movementData.quantity < 0 || (!isAdjustment && movementData.quantity === 0)) {
      throw new Error("Quantité invalide.");
    }
    const allowed = ["ENTREE_LIVRAISON", "SORTIE_CONSOMMATION_CHANTIER", "PERTE_CASSE", "AJUSTEMENT_INVENTAIRE"];
    if (!allowed.includes(movementData.movementType)) throw new Error("Type de mouvement non pris en charge.");
    const db = await IdbAdapter.getDb();
    return new Promise((resolve, reject) => {
      const tx = db.transaction([IdbAdapter.STORES.INVENTORY_ITEMS, IdbAdapter.STORES.STOCK_MOVEMENTS], "readwrite");
      const items = tx.objectStore(IdbAdapter.STORES.INVENTORY_ITEMS);
      const request = items.get(movementData.itemId);
      let result: StockMovementEntity;
      let failure = "Échec d’enregistrement du mouvement.";
      tx.onabort = () => reject(new Error(failure));
      tx.oncomplete = () => resolve(result);
      request.onsuccess = () => {
        try {
          const item = request.result as InventoryItemEntity | undefined;
          if (!item || item.deletedAt || !ProjectScope.visible("inventory_items",item,scope)) throw new Error("Article introuvable.");
          const quantity = movementData.quantity;
          const stock = isAdjustment ? quantity : movementData.movementType === "ENTREE_LIVRAISON"
            ? item.currentStock + quantity : item.currentStock - quantity;
          if (!Number.isFinite(stock) || stock < 0) throw new Error("Stock insuffisant pour cette sortie.");
          const now = new Date().toISOString();
          result = {...movementData, id: `mov_${crypto.randomUUID()}`, createdAt: now, updatedAt: now, syncStatus: "local"};
          items.put({...item, currentStock: stock, totalStockValueFCFA: stock * item.unitPurchasePriceFCFA,
            isBelowAlertThreshold: stock <= item.minStockAlert, updatedAt: now, syncStatus: "local",
            lastRestockedDate: movementData.movementType === "ENTREE_LIVRAISON" ? movementData.date : item.lastRestockedDate});
          tx.objectStore(IdbAdapter.STORES.STOCK_MOVEMENTS).put(result);
        } catch (error) {
