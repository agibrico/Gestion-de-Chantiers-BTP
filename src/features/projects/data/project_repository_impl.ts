...
export class ProjectRepositoryImpl implements IProjectRepository {
  ...
  public async getAllProjects(query?: ProjectFilterQuery): Promise<ProjectEntity[]> {
    ...
    const expenses = await IdbAdapter.getAll<any>(IdbAdapter.STORES.EXPENSES);
    projects = projects.map(project => ({...project, totalExpensesRealized: expenses
      .filter(e => e.projectId === project.id && ['APPROUVE','PAYE'].includes(e.status))
      .reduce((total, e) => total + (Number.isFinite(e.amountFCFA) ? e.amountFCFA : 0), 0)}));
  ...
  public async createProject(dto: CreateProjectDTO): Promise<ProjectEntity> {
    ...
    const newProject: ProjectEntity = {
      ownerId: typeof localStorage !== "undefined" ? localStorage.getItem("agb_current_user_id") || undefined : undefined,
      dossierState: "VALIDATED",
      initialReference: {budget:dto.totalBudgetEstimated,market:dto.totalBudgetContracted,start:dto.startDate,end:dto.estimatedEndDate,validatedAt:now},
      id,
      code,
      name: dto.name,
      description: dto.description,
      type: dto.type,
      status: dto.status || "ETUDE_PREPARATION",
  ...
  public async initializeSeedData(): Promise<void> {
    ...
    const count = (await IdbAdapter.getAll(this.storeName, true)).length;
    if (count > 0) return;
